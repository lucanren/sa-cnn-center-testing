...
class net_one_neuron_sa_central_halfumap(nn.Module):
...
class seperate_core_model_sa_central_halfumap(nn.Module):
    def __init__(self,num_neurons,q_dim,k_dim,v_dim,v_mapping):
        super().__init__()
        self.models = nn.ModuleList([net_one_neuron_sa_central_halfumap(q_dim=q_dim,k_dim=k_dim,v_dim=v_dim,v_mapping=v_mapping) for i in range(num_neurons)])
